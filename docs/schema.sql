...
CREATE TABLE `group_user_relation` (
  ...
  `user_id` bigint(20) NOT NULL COMMENT '用户Id',
  `group_name_remark` varchar(20) DEFAULT NULL COMMENT '群组名称备注',
  `join_time` datetime DEFAULT NULL COMMENT '加入时间',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='群组-用户关系表';
...
CREATE TABLE `history_msg` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `talk_id` bigint(20) NOT NULL COMMENT '会话Id',
  `talk_type` tinyint(2) NOT NULL COMMENT '会话类型',
...
CREATE TABLE `user_apply` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `main_user_id` bigint(20) NOT NULL COMMENT '自己的用户Id',
  `sub_user_id` bigint(20) NOT NULL COMMENT '好友的用户Id',
  ...
  `deleted` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '好友申请表';
...
CREATE TABLE `user_relation` (
  ...
  `sub_user_id` bigint(20) NOT NULL COMMENT '好友的用户Id',
  `talk_id` bigint(20) NOT NULL COMMENT '会话Id',
  `sub_nickname_remark` varchar(20) DEFAULT NULL COMMENT '好友昵称备注',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `user_relation_main_user_id_sub_user_id_uindex` (`main_user_id`,`sub_user_id`)
