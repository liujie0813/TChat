...
class Home extends Component {
	...
	onChatPage = e => {
		...
		let contactPageMenu = this.state.contactPageMenu;
		if (Object.keys(contactPageMenu).length !== 0) {
			contactPageMenu.target.src = img4;
		}
		e.target.src = img3;
	...
	onContactPage = e => {
		...
		let chatPageMenu = this.state.chatPageMenu;
		if (Object.keys(chatPageMenu).length !== 0) {
			chatPageMenu.target.src = img2;
		}
		e.target.src = img5;
							...
							<div style={{ height: '100%', width: '270px', backgroundColor: '#f7f7f7', borderRight: 'solid 1px #e0e0e0', float: 'left' }}>
								<div style={{ borderBottom: 'solid 1px #e0e0e0', height: '60px' }}>
									<Space direction="vertical" style={{ marginLeft: '10px', marginTop: '20px' }}>
										<Search placeholder="搜索" allowClear onSearch={this.onSearch} style={{ width: '250px' }}/>
									</Space>
								</div>
							</div>
